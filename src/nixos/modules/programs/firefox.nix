{...}: {
  flake.modules.homeManager.clj-firefox = {
    pkgs,
    lib,
    config,
    ...
  }: {
    options.clj.programs.firefox.enable = lib.mkEnableOption "Firefox" // {default = true;};
    config = lib.mkIf config.clj.programs.firefox.enable {
      home.persistence.${config.clj.impermanence.persistDir} = {
        directories = [
          # Capture the core profile data, history, and extensions
          ".mozilla/firefox"
          # Capture the cache (tabs, session restoration state, etc.)
          ".cache/mozilla"
        ];
      };
